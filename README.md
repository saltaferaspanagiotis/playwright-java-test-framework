...
`fixtures/PlaywrightFixture.java` owns the Playwright/Browser/BrowserContext/Page objects, one set per thread (`ThreadLocal`, required since scenarios run in parallel). Before each scenario it creates a fresh `BrowserContext` (with HAR recording) and a new `Page`, and starts Playwright tracing. After each scenario it takes a full-page screenshot (attached to the Allure report), stops the trace, and closes the context. `CucumberHooks` calls `PlaywrightFixture.initContext(...)`/`cleanupContext(...)` from Cucumber's own `@Before`/`@After` hooks (Cucumber scenarios don't go through JUnit's `@BeforeEach`/`@AfterEach`, so this is how the same setup/teardown logic applies to both).
